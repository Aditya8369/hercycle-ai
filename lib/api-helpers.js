...
export function predictNextPeriod(cycleHistory) {
  ...
  if (deduped.length < 2) {
    ...
    return {
      nextPeriodDate: formatDisplayDate(addDays(deduped[0].start_date, avgLen)),
      confidence: '75%',
  ...
  const lastPeriod = deduped[deduped.length - 1].start_date
  const formattedDate = formatDisplayDate(addDays(lastPeriod, avgLength))
