...
export function computeBackoffMs(attempts, random = Math.random) {
  ...
  // Full jitter with floor protection to prevent immediate retry thrashing.
  const jitterFloor = ceiling / 2
  const randVal = typeof random === 'function' ? random() : Math.random()
  const safeRand = Number.isFinite(randVal) ? Math.max(0, Math.min(1, randVal)) : 0.5
  return Math.round(jitterFloor + safeRand * jitterFloor)
}
